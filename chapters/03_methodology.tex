...
Real-$\Phi$-sat-2 and Sentinel-2 acquisition dates are matched to within a median of 6.8 days and a maximum of 14.9 days, not simultaneously, which is an acceptable temporal offset for a largely stable target such as land cover but a caveat worth stating explicitly.
...
However this introduces a different problem: because the split is made over patches, all 1{,}299 source products are represented in the training set, and every one of the 1{,}287 distinct products contributing to the test set also contributes patches to training (Figure~\ref{fig:splits}, right).
Since patches from the same product are spatially adjacent or overlapping crops of one acquisition, this means the test set is not a clean geographic or scene-level hold-out, and real-data performance figures reported in Chapter~\ref{chap:results-discussion} should be read as an optimistic upper bound on generalization to genuinely unseen scenes.
Re-splitting at product level instead of patch level is the direct remedy and is noted as future work in Section~\ref{sec:discussion}.
...
First, the raw radiometric scales of the two sensors heavily differ (Figure~\ref{fig:band-boxplots}): real $\Phi$-sat-2 digital numbers are concentrated an order of magnitude below Sentinel-2 and simulated-$\Phi$-sat-2, whose density curves overlap so closely in every band that the Sentinel-2B trace is entirely hidden beneath the simulated one -- a more direct confirmation of their near-identical raw statistics than a summary boxplot gives, since the full distributions, not just their quartiles, coincide.
Real $\Phi$-sat-2's distribution is not simply shifted lower, however: alongside its dominant low-value mode it carries a smaller secondary lobe that reaches into the same value range as Sentinel-2/simulated, overlapping with their peak in every band.
...
The simulated and raw Sentinel-2 domains remain close to statistically indistinguishable at this level, confirming that the physics-based degradation pipeline (Section~\ref{sec:degradation-pipeline}) perturbs fine spatial and spectral structure while leaving intensity statistics largely untouched.
...
This is a structural difference in distribution shape, not merely a difference in spread, and it is this shape mismatch -- not a location or scale offset that per-domain normalization already removes -- that the domain-adaptation mechanism of Section~\ref{sec:da-mechanism} must close.
...
Third, the residual disagreement after per-domain normalization can be localized spatially and broken down by land-cover class.
...
Because both views are already standardized on their own statistics, Equation~\eqref{eq:approx-error} isolates exactly the part of the gap that marginal alignment does not explain -- point spread function, band response, view geometry, and residual co-registration error, and not the location/scale offset the normalization protocol already removes.
...
Open, low-texture or specular surfaces -- bare ground, water -- are plausibly more sensitive to the residual radiometric and geometric mismatches Equation~\eqref{eq:approx-error} captures than vegetated, texturally rich surfaces, which anchor co-registration and view-geometry effects more consistently across sensors.
The three classes shown with the most extreme bars (moss and lichen, snow and ice, mangroves) each account for under $1\%$ of labelled pixels in this sample and carry correspondingly enormous error bars; they are reported for completeness but should not be read as robust estimates.
...
\begin{figure}[htp]
    ...
    \caption{Mean cross-sensor approximation error (Equation~\eqref{eq:approx-error}) by dominant ESA WorldCover class, over 5,000 patches.
        ...
        There are three classes at 0\% of presence in the samples (moss and lichen, snow and ice, mangroves) as they are rare in the dataset, but they are reported for completeness.
    }
...
Figures~\ref{fig:triplet-overlay-a}--\ref{fig:triplet-overlay-c} show three individual patches with the LULC mask overlaid on both sensor views alongside the approximation-error map of Equation~\eqref{eq:approx-error}, illustrating the range this dataset actually spans.
...
\begin{figure}[htp]
    ...
    \caption{A coastal wetland patch (mean approximation error $1.24$, above the dataset-wide mean of $0.91$).
        Errors are located over the over open water, consistent with the class-level pattern of Figure~\ref{fig:approx-error-class}.
    }
...
The scene content is identical across the ladder by construction, so any change in the base TerraMind encoder's response is attributable to the effect being varied, not to what the imagery depicts.
...
Each stage draws its parameters from its own intermediate input instead of a single fixed configuration, so an individual effect can be varied or disabled without disturbing the others, which is what permits the factor-isolation use described in Section~\ref{sec:sen1floods-sim}.
...
A model fine-tuned this way doesn't show lower domain gap to real $\Phi$-sat-2 than a model fine-tuned only on clean Sentinel-2.
...
Concretely, the two-branch distillation scheme (Section~\ref{sec:kd}) forwards a Sentinel-2 batch and a $\Phi$-sat-2 batch through the student separately in each training step; DSBN lets each pass use its own domain's normalization statistics instead of a single statistics set blended across both.
...
This argument follows from the documented conversion behaviour of the toolchain and not from a completed export of the student to the $\Phi$-sat-2 processor, which has not been carried out.
...
This guarantees a directly supervised alignment objective in place of an unpaired one.
...
This directly penalizes the encoder for representing sensor identity, not only scene content, and does so using the strongest available supervisory signal, exact scene correspondence, instead of matching marginal distributions between unpaired sensor domains as an MMD-style objective would.
...
Therefore, compression must cross architecture families, from ViT teacher to CNN student, instead of compressing within the ViT family as some prior on-board GeoFM work does\cite{du2025onorbitdemonstrationgeospatialfoundation}.
...
It was chosen for its simple encoder-decoder structure, which is a natural fit for the segmentation task, and for its low operator set requirements, which are compatible with the Myriad 2 VPU as demonstrated by the $\Phi$satNet baseline (Section~\ref{sec:phisatnet-background}).
...
This thesis adopts the second, for a reason independent of which specific mechanism is used.
The student model, is to be deployed, so whatever mechanism is used should be evaluated on the student's own representation and not assumed to transfer from the teacher.
...
This transfer of supervision across sensors is possible only because the two views are co-registered, and it keeps the teacher's own domain-shift error out of the student.
