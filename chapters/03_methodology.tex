...
We first describe the data used and the role each dataset plays, then the physics-based degradation pipeline and its calibration, then the protocol for measuring the domain gap, then the domain-adaptation mechanism adopted and the cross-architecture distillation procedure, and finally the evaluation protocol.
The chapter closes with the tooling and the reproducibility practices used throughout.

The data underlying this work falls into two categories.
...
This dataset carries land-cover labels derived from ESA WorldCover 2021, and it is the only real-sensor ground truth currently available to us.
The second category is simulated: a physics-degraded, graded version of the Sen1Floods flood-segmentation benchmark, produced with the same degradation pipeline.
It stands in for a real labeled $\Phi$-sat-2 flood dataset, which does not yet exist because the mission has not captured an annotated flood event.
