...
At a budget of $10{,}000$ labelled patches the distilled student reaches $0.4607$ best validation mIoU against the baseline's $0.3720$, an improvement of $+0.0887$ and $0.4454$ against $0.3739$ on the test split (Table~\ref{tab:main-results}).
...
\begin{figure}[htbp]
    ...
    \caption{Left: $\Phi$-sat-2 validation mIoU over training at $10{,}000$ labelled patches, with the best epoch marked. Dotted line marks teacher's accuracy on Sentinel-2.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrrrc}
        ...
        \midrule
        Distilled student (KD + DSBN) & $4.72$\,M  & $5.14$\,M      & $9.86$\,M          & $\mathbf{0.4454}$ \\
...
$\Phi$satNet gives up $0.042$ test mIoU against the supervised baseline and $0.114$ against the distilled student, for $28.1$ times fewer parameters.
...
It consumes the eight-band stack its input stem was built for, including the panchromatic band, where the other two models take the seven-band Blue--NIR stack of Section~\ref{sec:normalization}.
...
KD and UDA are usually motivated by a label scarcity in the target domain~\cite{kouw2019introductiondomainadaptationtransfer}, so we would expect the advantage above to grow as the target labels are removed; the opposite is what we observe.
