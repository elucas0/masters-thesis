...
The measurements above establish that a gap exists, and they locate it by layer in the low-level radiometry, but they cannot attribute it to a physical cause.
...
Because a single checkpoint is scored across all thirteen test splits, the drop can be paired within a seed, and this matters a lot here: the absolute IoU varies by up to $0.06$ between seeds while the paired drops agree to within $0.01$, so the pairing is what makes the smaller effects resolvable at all.
...
The spread across the configurations is threefold or more at every severity, against a near-constant spread for the misalignment, so the noise sensitivity is the factor that is the most amenable to being engineered away by a better or a larger representation.
...
The frozen configurations do show a small monotone response, which is negative at all twelve seed-and-severity combinations for the frozen TerraMind, so the pretrained representation is not entirely blur-agnostic.
But no individual severity clears the significance threshold in any column, and the effect stays an order of magnitude below the other two factors.
...
Distillation and unsupervised DA are usually motivated by a label scarcity in the target domain~\cite{kouw2019introductiondomainadaptationtransfer}, so we would expect the advantage above to grow as the target labels are removed.
