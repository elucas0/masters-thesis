...
This chapter reports the experiments that evaluate the three research questions of Section~\ref{sec:research-objective}: whether a measurable sensor domain gap exists in the latent space of a geospatial foundation model, where it concentrates and which physical sensor difference produces it, whether cross-architecture distillation from that model can produce a sensor-adaptive compact student, and whether doing so is preferable to simply training the compact model on the target sensor directly.
...
The epoch-to-epoch validation variance is of the order of $0.02$ \acref{mIoU}, so differences smaller than roughly $0.03$ should not be treated as resolved.
...
The WorldCover class prior on this dataset is severely imbalanced: the three rarest classes (snow/ice, mangroves, moss/lichen) together account for $1.2\%$ of labelled pixels but, under a macro-averaged \acref{mIoU}, for $27\%$ of the metric.
Under unweighted cross-entropy the student never predicted these classes at all, scoring exactly zero \acref{IoU} on them in training as well as validation.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lll}
        ...
        Early stopping          & \multicolumn{2}{l}{Patience $12$ on target-sensor validation \acref{IoU}}                                           \\
        Model selection         & \multicolumn{2}{l}{Best target-sensor validation \acref{IoU}}                                                       \\
...
One asymmetry in this comparison is structural rather than incidental.
...
\section{Degradation Factor effects on Downstream Task Gap}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        Clean baseline (\acref{IoU}) & $\mathbf{0.814} \pm 0.002$  & $0.739 \pm 0.057$  & $0.773 \pm 0.033$           \\
        \midrule
...
On clean data the distilled student is the strongest of the three, at $0.814$ \acref{IoU} against $0.773$ for fine-tuned TerraMind and $0.739$ for frozen TerraMind.
...
Band-to-band shifts corrupt precisely the cross-band relationships on which a water/land decision rests, and no amount of encoder capacity recovers information that the shift has already destroyed.
...
For fine-tuned TerraMind it is indistinguishable from zero: four severities spanning a twenty-four-fold reduction in image gradient energy move the score by at most $0.008$ \acref{IoU}.
...
The accuracy of the student on the two sensors converges on parity: on the test split it scores $0.4481$ on $\Phi$-sat-2 against $0.4640$ on Sentinel-2, which is a residual difference of $-0.016$ mIoU, and this against a teacher whose representation does not usefully transfer at all.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcc}
        ...
        \addlinespace
        \acref{IoU}($\Phi$-sat-2) $-$ \acref{IoU}(Sentinel-2), test  & ---               & $\mathbf{-0.004}$ \\
        \bottomrule
    \end{tabular}
...
\acref{PAD} is therefore not a valid instrument for a model that carries a domain-specific normalization, even though it is scale-free and hence otherwise well suited to comparing representations of different dimensionality.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{llccc}
        \toprule
        Labels                                                        & Method              & Val.\ \acref{mIoU}        & Test \acref{mIoU}         & Cost (h)       \\
        \midrule
...
At a budget of $10{,}000$ labelled patches the distilled student reaches $0.4607$ best validation mIoU against the baseline's $0.3720$, an improvement of $+0.0887$ and $0.4481$ against $0.3739$ on the test split (Table~\ref{tab:main-results}).
...
\begin{figure}[htbp]
    ...
    \caption{Left: $\Phi$-sat-2 validation \acs{mIoU} over training at $10{,}000$ labelled patches, with the best epoch marked. Dotted line marks teacher's accuracy on Sentinel-2.
...
This overtake could be read as 
...
Permanent water is the sole exception ($-0.013$).
The frequency-weighted \acref{IoU} moves in the same direction as the macro average ($0.523$ against $0.454$), so this result is not an artefact of the rare-class weighting introduced in Section~\ref{sec:configuration}.
...
\wip[Decide how much weight the two classes where the baseline wins deserve.]{%
    ...
    Permanent water is the only class where the undistilled baseline is ahead ($-0.013$), and it is spectrally distinctive and not rare enough for the class weighting to be doing the work, though a margin that size is not resolved by a single run (Section~\ref{sec:limitations}).
...
Separately, the teacher reaches $0.6351$ mIoU on Sentinel-2 while the student reaches $0.4481$ on $\Phi$-sat-2, so roughly $0.19$ mIoU of the teacher--student gap remains unclosed.
...
The comparison above holds the architecture fixed and varies only the training signal, so it says nothing about where either arm sits on the complexity axis.
$\Phi$satNet answers that question directly: at $0.351$\,M parameters it is a twenty-eighth of the U-Net that both arms use, and it is the class of model a mission would actually carry on board.
Table~\ref{tab:complexity} places them side by side, with $\Phi$satNet trained on the same $10{,}000$ labelled patches, the same split, the same focal loss and class weighting, the same optimiser, schedule and early-stopping criterion as the supervised baseline, and in two initialisations: from the released land-cover checkpoint, and from scratch.

\begin{table}[htbp]
    \centering
    \begin{tabular}{lrrrc}
        \toprule
        Model                         & Encoder    & Decoder + head & Total              & Test \acref{mIoU}         \\
        \midrule
        Distilled student (KD + DSBN) & $4.72$\,M  & $5.14$\,M      & $9.86$\,M          & $\mathbf{0.4481}$ \\
        Supervised baseline U-Net     & $4.72$\,M  & $5.14$\,M      & $9.85$\,M          & $0.3739$          \\
        $\Phi$satNet, pre-trained     & $199$\,k & $152$\,k     & $\mathbf{0.351}$\,M & $0.3316$          \\
        $\Phi$satNet + KD + DSBN      & $199$\,k & $152$\,k     & $0.352$\,M          & $0.3268$          \\
        $\Phi$satNet, random init     & $199$\,k & $152$\,k     & $\mathbf{0.351}$\,M & $0.3215$          \\
        \bottomrule
    \end{tabular}
    \caption{Trainable parameters against accuracy at $10{,}000$ labelled patches.
        ...
        Test mIoU is on the full $25{,}323$-patch test split, on the fixed eleven-class denominator of Section~\ref{sec:measurement-pitfalls}.
        The frozen teacher ($94.4$\,M) and the projection heads ($0.34$\,M) are training-time cost only and are not carried at inference, so neither appears here.
    }
    \label{tab:complexity}
\end{table}

The accuracy ordering follows the parameter ordering, but very far from proportionally.
$\Phi$satNet gives up $0.042$ test mIoU against the supervised baseline and $0.117$ against the distilled student, for $28.1$ times fewer parameters.
Per million trainable parameters it returns $0.944$ mIoU against the baseline's $0.038$ and the distilled student's $0.045$, a factor of roughly twenty-five.
The distilled student's own margin, by contrast, is bought at almost no parameter cost at all: the domain-specific normalization adds $6{,}848$ parameters to the shared architecture, $0.07\%$ of the total, and the two U-Net arms are otherwise identical in size (Section~\ref{sec:dsbn-contribution} treats how much of the accuracy gain that mechanism accounts for).

Distilling into $\Phi$satNet does not move it.
Running the distillation of Section~\ref{sec:rq3-results} unchanged --- same teacher, same objective, same domain-specific normalization, same label budget --- but with $\Phi$satNet in place of the U-Net student yields $0.3268$ test mIoU, which is $0.005$ \emph{below} the same architecture trained supervised and $0.005$ above it from a random initialisation.
All three $\Phi$satNet configurations sit inside a band of $0.01$, while the identical intervention is worth $+0.074$ to the U-Net.
The teacher is not the difference: its own score on the source sensor is $0.6351$ in both distillation runs.

The natural reading is a capacity ceiling.
...
One difference keeps the $\Phi$satNet rows from being a strict architectural control: they consume the eight-band stack their input stem was built for, including the panchromatic band, where the other two models take the seven-band Blue--NIR stack of Section~\ref{sec:normalization}.

The two $\Phi$satNet rows differ only in their initialisation, and together they say what the mission-specific pre-training is worth on this task: very little.
Fine-tuning from the released land-cover checkpoint, itself pre-trained on \emph{simulated} $\Phi$-sat-2 imagery, beats the same architecture from a random initialisation by $0.0102$ test mIoU.
The per-class pattern does not look like a systematic lift either: the pre-trained run leads on six of the eleven classes and trails on five, and only cropland ($+0.070$) and snow/ice ($+0.065$) move by more than $0.03$ either way.
Two further observations point the same direction.
Run on real $\Phi$-sat-2 with no training at all, the released land-cover model scores between $0.0008$ and $0.0413$ mIoU across the ten published checkpoints, against the $0.0376$ that a uniform-random predictor scores on these eleven class frequencies; and a frozen-encoder sweep over those same checkpoints did not separate any of them from a random initialisation.
On this task and at this budget it is therefore the architecture, and not the pre-training corpus, that accounts for the $\Phi$satNet result.
Each configuration is a single seed, so a margin of $0.01$ is not resolved by these runs (Section~\ref{sec:limitations}).
...
At $1{,}000$ labelled patches the supervised baseline is ahead by every measure: best validation \acref{mIoU} $0.2916$ against $0.2679$, test \acref{mIoU} $0.2707$ against $0.2635$, and on matched epochs the distilled student leads in only $8$ of the $58$ epochs both runs share.
...
The reversal is consistent across every measure reported (best validation \acref{mIoU}, test \acref{mIoU}, and the epoch-by-epoch lead), but the two ends of the crossover are not equally well resolved.
At $10{,}000$ labels the margin ($0.0618$ validation \acref{mIoU}) is larger than the epoch-to-epoch variance noted at the start of this chapter; at $1{,}000$ labels it is not ($0.0237$ validation, $0.0072$ test), and the baseline's advantage at the small budget therefore rests mainly on the consistency of the epoch-by-epoch lead, not on the size of the final gap.
...
First, the macro-averaged \acref{mIoU} implemented by standard libraries excludes classes with zero union from its average, so its denominator depends on which classes a given model happens to predict.
Where a rare class is absent from a small evaluation split, two models are then averaged over a different number of classes and their \acref{mIoU} values are simply not comparable.
...
Second, an evaluation split sized as a fixed fraction of the training split makes a label-scarcity study vary its measurement and its treatment at the same time.
...
The student is trained from a random initialisation because no pre-trained weights exist for this backbone, and this plausibly accounts for a substantial share of the remaining $0.15$ \acref{mIoU} gap to the teacher, and it confounds any claim that this gap is attributable to a domain shift.
...
\wip{The flood-segmentation evaluation is only partially complete at the time of writing, and the scope of what follows should be read accordingly.
    ...
    The invariance and label-budget findings above therefore still rest on the \acref{LULC} task alone, and no claim is made yet about whether they carry over to flood segmentation.
}

The mild flood-\acref{IoU} degradation observed for the teacher across the simulated degradation ladder (Section~\ref{sec:ladder-results}, at worst $-0.152$ \acref{IoU} under full-severity noise and $-0.008$ under full-severity blur) is not unique to this thesis.
...
Flood segmentation may then understate the severity of the domain-gap problem, and this is part of why we adopted the \acref{LULC} task as the primary benchmark.
...
On this task the $\Phi$satNet \emph{pre-training} contributes almost nothing: fine-tuning from the released land-cover checkpoint beats the identical architecture from a random initialisation by $0.0102$ test mIoU, and that same checkpoint, run on real $\Phi$-sat-2 without any training, scores at the level of a uniform-random predictor.
...
It is, however, the same problem this thesis is about, displaced one level: the released $\Phi$satNet weights were pre-trained on \emph{simulated} $\Phi$-sat-2 imagery, and Section~\ref{sec:rq1-results} shows that a gap of this kind originates in low-level radiometry rather than in semantics, which is exactly the part of a sensor a simulator has to get right and the part it is hardest to get right.
...
First, the missions intended to host on-board foundation models should prioritise collecting co-registered calibration data early in their operational life. The alignement with some existing acquisition from Sentinel-2 made the measurements of this chapter possible.
