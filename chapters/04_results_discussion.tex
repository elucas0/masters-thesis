...
The exception is the degradation-ladder evaluation of Section~\ref{sec:ladder-results}, which was replicated over three seeds and for which drops are reported with a measured seed-to-seed deviation; that section is consequently the only one in this chapter whose smaller effects are resolved.
...
The measurements above establish that a gap exists and locate it, by layer, in low-level radiometry, but they cannot attribute it to a physical cause: the two instruments differ in spectral response, radiometric noise, band registration and ground sampling distance simultaneously, and no measurement on real pairs can separate those.
...
\begin{table}[htbp]
    \centering
    \caption{The degradation ladder. Level~4 reproduces the joint simulator configuration; each step down halves the perturbation. SNR is drawn per scene from the stated range and the noise amplitude scales as its reciprocal, so the four ranges correspond to relative noise of $1/8$, $1/4$, $1/2$ and $1$. Blur and misalignment parameters are in resampled $4.75$\,m pixels.}
...
What is measured is the domain gap proper: each model is trained once on the undegraded rung and the resulting checkpoint is then evaluated on every rung's test split, so the degradation appears only at inference, as it would for a model trained on Sentinel-2 and deployed on $\Phi$-sat-2.
...
Each configuration is run for three seeds.
Because a single checkpoint is scored across all thirteen test splits, the drop can be paired within seed, which matters here: absolute IoU varies by up to $0.06$ between seeds while the paired drops agree to within $0.01$, so pairing is what makes the smaller effects resolvable at all.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        Clean baseline (IoU) & $0.835 \pm 0.011$ & $0.739 \pm 0.057$ & $0.773 \pm 0.033$ \\
        \midrule
        \multicolumn{4}{l}{\emph{Sensor noise (SNR)}} \\
        \quad L1 & $-0.047 \pm 0.030$            & $+0.002 \pm 0.003$ & $-0.001 \pm 0.001$            \\
        \quad L2 & $-0.144 \pm 0.049^{\ast}$     & $+0.003 \pm 0.006$ & $-0.004 \pm 0.002$            \\
        \quad L3 & $-0.260 \pm 0.039^{\ast}$     & $-0.018 \pm 0.017$ & $-0.041 \pm 0.014^{\ast}$     \\
        \quad L4 & $\mathbf{-0.455} \pm 0.058^{\ast}$ & $-0.088 \pm 0.066$ & $-0.152 \pm 0.030^{\ast}$ \\
        \addlinespace
        \multicolumn{4}{l}{\emph{Optical blur (PSF)}} \\
        \quad L1 & $-0.001 \pm 0.003$        & $-0.004 \pm 0.003$ & $0.000 \pm 0.000$ \\
        \quad L2 & $-0.010 \pm 0.010$        & $-0.010 \pm 0.007$ & $0.000 \pm 0.001$ \\
        \quad L3 & $-0.042 \pm 0.023$        & $-0.016 \pm 0.012$ & $0.000 \pm 0.003$ \\
        \quad L4 & $-0.128 \pm 0.063^{\ast}$ & $-0.025 \pm 0.016$ & $\mathbf{-0.008} \pm 0.009$ \\
        \addlinespace
        \multicolumn{4}{l}{\emph{Band misalignment}} \\
        \quad L1 & $-0.006 \pm 0.001^{\ast}$ & $-0.008 \pm 0.003^{\ast}$ & $-0.003 \pm 0.001^{\ast}$ \\
        \quad L2 & $-0.033 \pm 0.004^{\ast}$ & $-0.033 \pm 0.005^{\ast}$ & $-0.013 \pm 0.003^{\ast}$ \\
        \quad L3 & $-0.084 \pm 0.013^{\ast}$ & $-0.065 \pm 0.005^{\ast}$ & $-0.051 \pm 0.007^{\ast}$ \\
        \quad L4 & $\mathbf{-0.150} \pm 0.015^{\ast}$ & $\mathbf{-0.119} \pm 0.009^{\ast}$ & $\mathbf{-0.109} \pm 0.012^{\ast}$ \\
        \bottomrule
    \end{tabular}
\end{table}
...
Its full-severity cost varies only between $-0.109$ and $-0.150$ IoU across models that differ by an order of magnitude in trainable parameters and by architecture family, and it is the sole factor resolved at the mildest severity in all three columns, where a $1.25$-pixel shift already costs a small but reproducible amount.
Its seed-to-seed deviation is also the tightest in the table.
A perturbation whose cost is that insensitive to the model absorbing it is best read as damage to the input signal rather than as a weakness of any particular representation: band-to-band shifts corrupt precisely the cross-band relationships on which a water/land decision rests, and no amount of encoder capacity recovers information that the shift has destroyed.

Sensor noise produces the largest single degradation, but its magnitude is a property of the model rather than of the perturbation.
It costs the distilled student $-0.455$ IoU at full severity, fine-tuned TerraMind $-0.152$, and frozen TerraMind $-0.088$ with a deviation ($0.066$) large enough that the frozen figure is not resolved at three seeds.
The spread across configurations is threefold or more at every severity, against the near-constant spread for misalignment, so noise sensitivity is the factor most amenable to being engineered away by a better or larger representation.

Optical blur is the least damaging factor throughout, and for fine-tuned TerraMind it is indistinguishable from zero: four severities spanning a twenty-four-fold reduction in image gradient energy move the score by at most $0.008$ IoU.
The frozen configurations do show a small monotone response, negative at all twelve seed-and-severity combinations for frozen TerraMind, so the pretrained representation is not entirely blur-agnostic; but no individual severity clears the significance threshold in any column, and the effect is an order of magnitude below the other two factors.

This ordering answers the attribution question left open in Section~\ref{sec:rq1-results}, and it answers it in the same direction.
The factor that models a resolution difference, optical blur, is the one that costs almost nothing, while the radiometric factor and the band-registration factor carry the effect.
...
It also reinforces the point that the gap here is not resolution-driven, which is the reverse of the emphasis in related on-orbit work~\cite{du2025onorbitdemonstrationgeospatialfoundation}.
...
Repeating the sweep with each model trained on the same degraded condition it is evaluated on, rather than on clean data, recovers most of the loss (Table~\ref{tab:ladder-adaptation}): $84\%$ of the noise penalty and $66\%$ of the misalignment penalty disappear, and blur ceases to be a penalty at all, scoring marginally \emph{above} the clean baseline in the manner of a mild augmentation.
...
Where the operating sensor's noise characteristics are known in advance, which for a launched instrument they are, simulating them during training reclaims most of the achievable performance without any domain-adaptation machinery.
...
The factors are varied one at a time, whereas a real sensor pair presents all of them at once and possibly interacting; the ladder identifies which effects matter in isolation, not how they compose.
The degradations are simulated, so the conclusion concerns the simulator's model of the instrument difference rather than the instrument difference itself, and the absence of real labelled $\Phi$-sat-2 flood data (Section~\ref{sec:sen1floods-sim}) is what makes that unavoidable.
...
The student's accuracy on the two sensors converges on parity: on the test split it scores $0.4454$ on $\Phi$-sat-2 against $0.4417$ on Sentinel-2, a residual difference of $-0.004$ mIoU, against a teacher whose representation does not usefully transfer at all.
...
\wip[Confirm you are comfortable defending the asymmetry in the probe row before this goes in.]{%
Placeholder for a remark on the one row that cuts against the student.
The teacher is the stronger encoder \emph{within} Sentinel-2 ($0.754$ against $0.663$ same-domain probe accuracy); its advantage disappears only when the probe is asked to cross the sensor gap.
Whether that is worth foregrounding as a limitation of the student or as the precise statement of what distillation bought is a judgement call, and it is yours to make.}
...
The control is a student of identical architecture trained directly on labelled $\Phi$-sat-2 with no teacher, no second sensor and no distillation, holding constant the split, normalization, augmentation, task loss, optimiser, schedule and early-stopping criterion (Table~\ref{tab:hyperparameters}).
...
Figure~\ref{fig:dsbn-vs-baseline} shows the shape of that margin: the baseline is ahead for roughly the first twenty-five epochs, the two cross, and the distilled run then separates and continues climbing after the baseline has flattened.
Averaged over the $71$ epochs the two runs share, the distilled student leads by $0.0161$ mIoU and is ahead in $49$ of them.

\wip[Read the paragraph above and confirm the framing matches your reading of the curves.]{%
Placeholder for your own interpretation of \emph{why} the distilled run overtakes late rather than early.
The mechanism is not established by these runs: the candidate explanations are that the teacher's signal is most useful once the student has enough capacity committed to the easy classes, or simply that the two-branch arm sees twice the supervision per epoch and therefore trails on wall-clock but not on epochs.
State whichever you are prepared to defend, or leave it as an open question.}

The distilled run had not converged.
Its best epoch is $89$ of the $101$ it ran, and it exhausted the full budget without triggering early stopping, so $0.4607$ should be read as a lower bound on what this configuration reaches.
The baseline, by contrast, early-stopped at epoch $80$.

Per-class results (Figure~\ref{fig:per-class}) show the gain is broad, not concentrated in one class.
...
Frequency-weighted IoU moves in the same direction as the macro average ($0.523$ against $0.454$), so the result is not an artefact of the rare-class weighting introduced in Section~\ref{sec:configuration}.

\wip[Fill in once you have decided how much weight to put on the two classes where the baseline wins.]{%
Placeholder for the per-class discussion.
Permanent water is the only class where the undistilled baseline is meaningfully ahead ($-0.019$), with shrubland level; it is spectrally distinctive and not rare enough for the class weighting to be doing the work.
A sentence on whether that pattern is meaningful, or within the per-class noise of a single seed, belongs here.}

Two costs qualify the result.
Distillation requires $7.9$ minutes per epoch against the baseline's $3.3$, a factor of $2.4$, dominated by the frozen teacher's forward pass and the second student branch; at a fixed compute budget rather than a fixed epoch count the comparison is much closer.
...
That residual is a compression gap rather than a domain gap: Section~\ref{sec:rq2-results} shows the student performs almost identically on the two sensors, so what separates it from the teacher is capacity and initialisation, not sensor mismatch.
...
The size of that shift is worth noting on its own: the same frozen teacher scores $0.5788$ on the $997$-patch subset and $0.6351$ on the full split, so subset-to-subset differences of this magnitude are not negligible and no number from the two evaluation regimes should be compared directly.
