...
Under unweighted cross-entropy the student never predicted these classes at all, scoring exactly zero \acref{IoU} on them in training as well as validation, a failure to learn, not overfitting.
...
Its full-severity cost varies only between $-0.109$ and $-0.140$ \acref{IoU} across models that differ by an order of magnitude in trainable parameters and by architecture family, and it is the sole factor resolved at the mildest severity in all three columns, where a $1.25$-pixel shift already costs a small but reproducible amount.
...
At full severity it costs the distilled student $-0.458$ \acref{IoU} on average and fine-tuned TerraMind $-0.152$, a threefold difference that holds at every severity and stands in contrast to the near-constant cost of misalignment.
...
The accuracy of the student on the two sensors converges on parity: on the test split it scores $0.4454$ on $\Phi$-sat-2 against $0.4417$ on Sentinel-2, which is a residual difference of $-0.004$ \acref{mIoU}, and this against a teacher whose representation does not usefully transfer at all.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{llccc}
        ...
        \midrule
        $10{,}000$                                                    & \acref{KD} + \acref{DSBN}           & $\mathbf{0.4607}$ & $\mathbf{0.4454}$ & $13.3$         \\
...
At a budget of $10{,}000$ labelled patches the distilled student reaches $0.4607$ best validation \acref{mIoU} against the baseline's $0.3720$, an improvement of $+0.0887$ and $0.4454$ against $0.3739$ on the test split (Table~\ref{tab:main-results}).
...
Separately, the teacher reaches $0.6351$ \acref{mIoU} on Sentinel-2 while the student reaches $0.4454$ on $\Phi$-sat-2, so roughly $0.19$ \acref{mIoU} of the teacher--student gap remains unclosed.
...
Table~\ref{tab:complexity} places the three side by side, with $\Phi$satNet trained on the same $10{,}000$ labelled patches, the same split, the same focal loss and class weighting, the same optimiser, schedule and early-stopping criterion as the supervised baseline.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrrrc}
        ...
        \midrule
        Distilled student (\acref{KD} + \acref{DSBN}) & $4.72$\,M  & $5.14$\,M      & $9.86$\,M          & $\mathbf{0.4454}$ \\
        Supervised baseline U-Net     & $4.72$\,M  & $5.14$\,M      & $9.85$\,M          & $0.3739$          \\
        $\Phi$satNet                  & $0.199$\,M & $0.152$\,M     & $\mathbf{0.351}$\,M & $0.3316$          \\
        \bottomrule
    \end{tabular}
    \caption{Trainable parameters against accuracy at $10{,}000$ labelled patches.
        Every parameter is trained in all three rows, so the totals are also the trainable counts.
        Test \acs{mIoU} is on the full $25{,}323$-patch test split, on the fixed eleven-class denominator of Section~\ref{sec:measurement-pitfalls}.
...
$\Phi$satNet gives up $0.042$ test \acref{mIoU} against the supervised baseline and $0.114$ against the distilled student, for $28.1$ times fewer parameters.
Per million trainable parameters it returns $0.944$ \acref{mIoU} against the baseline's $0.038$ and the distilled student's $0.045$, a factor of roughly twenty-five.
...
Two differences keep the $\Phi$satNet row from being a strict architectural control.
It consumes the eight-band stack its input stem was built for, including the panchromatic band, where the other two models take the seven-band Blue--\acref{NIR} stack of Section~\ref{sec:normalization}.
And it is not trained from a random initialisation: it is fine-tuned end to end from the released land-cover checkpoint, which was itself pre-trained on \emph{simulated} $\Phi$-sat-2 imagery.

\wip[Run the matched from-scratch control before this paragraph says anything about pre-training.]{%
    To write: whether $\Phi$satNet's $0.3316$ owes anything to its pre-training.
    The row as it stands measures the architecture and the pre-training together and cannot separate them, because the matched control at this configuration but from a random initialisation has not been run.
    Two prior observations make the null outcome a live possibility: the released land-cover model scores at chance on real $\Phi$-sat-2 before any training, and a short frozen-encoder sweep did not separate the pre-trained encoders from a random initialisation.
}
...
$\Phi$satNet (Section~\ref{sec:phisatnet-background}) reports that a purpose-built, non-transformer model trained from scratch on a mission-specific corpus matches or exceeds unconstrained transformer-based \acref{GeoFM}s on several on-board tasks~\cite{phisatnet}.
