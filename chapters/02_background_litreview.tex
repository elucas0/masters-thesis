...
The second is the lack of data as there wasn't enough imagery from the HyperScout product line to train a network to label every pixel as cloudy or clear.
...
The generalization claim behind GeoFMs has recently been put to a systematic test.
PANGAEA~\cite{marsocci2024pangaea} was created because existing GeoFM evaluations were narrow and inconsistent as benchmarks were often geographically biased toward North America and Europe, and several assessed models through a single instrument.
PANGAEA spans eleven datasets across urban, agricultural, marine, and forest domains, with global coverage, spatial resolutions from 1.5 to 30\,m per pixel, uni- to multi-temporal configurations, and several sensor types including multispectral, natural-colour, and synthetic aperture radar imagery.
The models are compared against supervised task-specific baselines, among them a plain U-Net and a vanilla ViT.

A relevant finding for this thesis is that GeoFMs do not consistently outperform those supervised baselines, and that task-specific models exploit the spectral and spatial detail of a dataset more effectively when the characteristics of the pre-training data differ from those of the downstream data.
The evaluation identifies the spectral and spatial resolution of the pre-training sensor as a significant determinant of downstream performance, especially when the downstream task requires fine-grained spatial or spectral information.
Another point relevant is the scope of what remains untested as the authors observe that geospatial models are evaluated mainly on narrow selections of tasks and datasets, leaving their ability to handle domain shifts across geographic regions, sensors, and temporal conditions largely unexplored.

PANGAEA compares across sensors in the sense that its datasets originate from different instruments, but the sensor is never the only variable that changes between them.
...
The problem is inherent to on-board AI, where the model is fixed at uplink time while acquisitions continue to vary with illumination, atmosphere, season, geography, and the behaviour of the instrument itself.
It is useful to separate three kinds of shift, since they have different causes can be minimized in different ways\cite{kouw2019introductiondomainadaptationtransfer}.
...
Domain-Specific Batch Normalization (DSBN) \cite{chang2019dsbn} is a generic technique originally for unsupervised domain adaptation, and is designed to be integrated into various algorithms based on deep neural networks with batch normalization layers.
Batch normalization (BN)\cite{ioffe2015batchnorm} is a technique used in deep learning to normalize the inputs of each layer, which helps stabilize and accelerate training.
DSBN extends BN by maintaining separate sets of normalization parameters (mean and variance) for each domain by keeping a single set of convolutional weights but a separate batch-normalization affine transform and running statistics per domain.
...
The simulator developed for the ESA $\Phi$-lab OrbitalAI challenge\cite{longepe2024orbitalai} models this transformation as a sequence of physical operations in radiance space, calibrated against real flight data from related missions. 
Spectral and spatial resampling to the target sensor's band configuration and ground sampling distance, band-to-band misalignment consistent with push-broom acquisition geometry, signal-to-noise degradation, and optical blur modeled through the sensor's modulation transfer function.
...
Their processing chain is the one described above: conversion to top-of-atmosphere radiance, a $\Phi$-sat-2-specific panchromatic band synthesised from the multispectral bands by spectral-response-weighted combination, spatial resampling to the target ground sampling distance, band-to-band misalignment, signal-to-noise degradation, modulation transfer function blurring, and conversion back to reflectance.
They trained on simulated $\Phi$-sat-2 patches and deployed the models on the same Intel Myriad 2 accelerator this thesis targets, which makes the work a useful independent confirmation that the simulator can support a full development cycle from a mission that has not yet delivered labelled data.

This study also illustrates the limit of the approach.
Its performance was established on simulated imagery throughout, and its authors identify validating the system on in-flight data, together with sensitivity testing against individual sensor phenomena such as noise and spectral deregistration, as work still to be done.
The simulator makes pre-launch development possible; on its own it cannot establish how far the simulated sensor stands in for the real one, which is the question the co-registered real acquisitions of Chapter~\ref{chap:methodology} are assembled to answer.
...
A matched few-shot comparison against $\Phi$satNet on a shared task and label budget remains the appropriate test of the label-efficiency claim; Chapter~\ref{chap:results-discussion} returns to this positioning, but that comparison has not been run and is left for future work.
